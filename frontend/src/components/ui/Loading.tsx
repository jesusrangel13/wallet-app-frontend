...
export function LoadingOverlay({
  ...
  return (
    <div
      ...
      aria-busy="true"
      className={cn(
        'absolute inset-0 bg-background/80 backdrop-blur-sm flex flex-col items-center justify-center z-10 rounded-lg gap-2',
        className
      )}
    >
      <LoadingSpinner size={size} aria-hidden="true" />
      {displayMessage && <p className="text-muted-foreground text-sm font-medium">{displayMessage}</p>}
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-4 p-4" role="status" aria-label="Loading row">
      {Array.from({ length: columns }).map((_, i) => (
        <div key={i} className="animate-pulse bg-muted rounded h-4 flex-1" aria-hidden="true" />
      ))}
    </div>
  )
}
...
export function SkeletonCard() {
  return (
    <div className="bg-card p-6 rounded-xl shadow-sm border border-border space-y-4" role="status" aria-label="Loading card">
      <div className="animate-pulse bg-muted rounded h-6 w-1/3" aria-hidden="true" />
      <div className="animate-pulse bg-muted rounded h-4 w-2/3" aria-hidden="true" />
      <div className="animate-pulse bg-muted rounded h-4 w-1/2" aria-hidden="true" />
    </div>
  )
}
