      ...
      <CardContent>
        {data.length > 0 ? (
          <div className="space-y-3">
            {/* Summary cards - Optimized for minHeight 2 */}
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-green-50 rounded-lg p-2 border border-green-100">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <TrendingUp className="h-3.5 w-3.5 text-green-600" />
                  <p className="text-xs text-green-700 font-medium">Ingreso Promedio</p>
                </div>
              ...
              <div className="bg-red-50 rounded-lg p-2 border border-red-100">
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  <p className="text-xs text-red-700 font-medium">Gasto Promedio</p>
                </div>
                <p className="text-base font-bold text-red-900">
                  {formatCurrency(avgExpense, 'CLP')}
                </p>
              </div>
              <div className={`${avgBalance >= 0 ? 'bg-blue-50 border-blue-100' : 'bg-red-50 border-red-100'} rounded-lg p-2 border`}>
                <div className="flex items-center gap-1.5 mb-0.5">
                  <DollarSign className={`h-3.5 w-3.5 ${avgBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`} />
                  <p className={`text-xs font-medium ${avgBalance >= 0 ? 'text-blue-700' : 'text-red-700'}`}>Balance Neto</p>
                </div>
                <p className={`text-base font-bold ${avgBalance >= 0 ? 'text-blue-900' : 'text-red-900'}`}>
